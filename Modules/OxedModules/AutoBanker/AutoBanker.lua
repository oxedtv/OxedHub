    ...
    for bag = 0, (NUM_TOTAL_EQUIPPED_BAG_SLOTS or NUM_BAG_SLOTS or 4) do
        for slot = 1, (C_Container.GetContainerNumSlots(bag) or 0) do
            ...
            if info and info.itemID and not info.isLocked then
                ...
                if wanted then
                    picks[#picks + 1] = { bag = bag, slot = slot, itemID = info.itemID }
                end
            end
        end
    end
...
local function DepositReagents()
    if not settings.reagents then return false end
...
local function OnBankOpened()
    ...
    local reagents = DepositReagents()
    ...
    if #picks == 0 then
        if reagents and settings.report then
            print(PREFIX .. "reagents deposited.")
        end
        return
    end
    ...
    Send(picks, free, function(moved)
        ...
        local parts = {}
        if reagents then parts[#parts + 1] = "deposited reagents" end
        if moved > 0 then
...
local function ShowOptions()
    ...
    if not optionsWindow then
        optionsWindow = API:CreateOptionsWindow("Auto Banker", 420, 240)
...
loginFrame:SetScript("OnEvent", function(self)
    ...
    OxedHub.ModuleAPI:Register({
        ...
        author   = "Oxed",
        category = "inventory",
        desc     = "Puts your reagents away when the bank opens, along with anything you already keep stored there. Hold Shift to skip.",
