-- ─────────────────────────────────────────────────────────────────────────────
-- PvPConsumable.lua — PvP trigger: Enemy Consumable Detection
--
-- Detects when your TARGET, FOCUS, or ARENA enemy uses a potion or consumable
-- (healing potion, refreshing serum, etc.).
--
-- Detection mechanism:
--   Uses the 12.1+ C_UnitAuras.AddAuraSound() API.
--
-- Spell database: OxedHub.PvPSpellDB.ConsumableSpells
-- ─────────────────────────────────────────────────────────────────────────────
...
local function GetConfiguredSounds()
    ...
    for id, trigger in pairs(OxedHub.db.profile.triggers) do
        if trigger.enabled and trigger.event == "PVP_CONSUMABLE" then
...
local function Refresh(reason)
    ...
    local spellDB = OxedHub.PvPSpellDB and OxedHub.PvPSpellDB.ConsumableSpells
    ...
    local disabledSpells = (activeTrigger and activeTrigger.conditions and activeTrigger.conditions.disabledConsumables) or {}
...
Triggers:RegisterEventType("PVP_CONSUMABLE", {
    name = L["EVT_PVP_CONSUMABLE"] or "Enemy Consumable Alert",
    ...
    CreateConditionUI = function(frame, trigger, yOffset)
        ...
        desc:SetText(
            "Plays a voice alert when your target, focus, or arena enemy uses a potion or consumable.\n\n"
        ...
        if OxedHub.UIComponents and OxedHub.UIComponents.PvPSpellPicker then
            yOffset = OxedHub.UIComponents.PvPSpellPicker:Create(frame, trigger, yOffset + 40, OxedHub.PvPSpellDB.ConsumableSpells, "disabledConsumables")
        end
        return yOffset
    end,
